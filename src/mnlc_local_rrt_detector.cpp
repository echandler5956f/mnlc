...
int main(int argc, char **argv)
{
    ...
    ros::Publisher shapes = n.advertise<visualization_msgs::Marker>("/mnlc_local_rrt_detector/shapes", 1);
    ros::Time st1;
    ...
    double ix = std::sqrt(std::pow(points.points[0].x - points.points[2].x, 2) + std::pow(points.points[0].y - points.points[0].y, 2));
    double iy = std::sqrt(std::pow(points.points[0].x - points.points[0].x, 2) + std::pow(points.points[0].y - points.points[2].y, 2));
    ...
    while (ros::ok())
    {
        ...
        if (out == -1)
        {
            ...
            while (flag == false)
            {
                try
                {
                    listener.lookupTransform("/map", "/base_footprint", ros::Time::now(), transform);
                    flag = true;
                }
